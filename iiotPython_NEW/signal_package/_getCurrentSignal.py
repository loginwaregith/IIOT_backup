...
import json
from ._globalVariables import PRODUCTION_ARRAY,LIVE_STATUS_CODES
...
def insertSignalToLocalDb(self,machineId,process,timeStamp):       
      ...
      try:
          if(curs2.execute(sql,values)):
              conn2.commit()
              logger.debug("successfully inserted into local database")
      except:
          logger.error("unable to insert into local database")

def productionOk(self,):
      timeObj = datetime.now()
      timeStamp=timeObj.strftime("%Y/%m/%d %H:%M:%S")
      data=curs2.execute("SELECT MAX(id) FROM production")
      lastId=curs2.fetchone()[0]
      sql="update production set status=? where id=?"
      values=("1", lastId)
      try:
          result=curr2.execute(sql,values)
          conn2.commit()
          logger.debug("updated status  1 to last production job ")
      except:
          logger.error("failed to update status  1 to last production job")
...
def updateLiveStatus(self,status,signal,color):
      try:
         ...
         conn2.commit()
         logger.debug("live status machine idle updated")  
      except Exception as e:
         logger.error("failed to update live status")          
